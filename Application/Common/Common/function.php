...
function chongzu($str){
	...
	return str_replace("BCC","BCH",str_replace('_','/',strtoupper($str)));
}
...
function convmarket($market){
	$arr = explode("_",$market);
	$str = implode("/",$arr);
	return str_replace("BCC","BCH",strtoupper($str));
}
function houmark($market){
	$arr = explode("_",$market);
	return $arr[1];
}
//bcc替换成bch
function tihuanb($name){
	return str_replace("BCC","BCH",strtoupper($name));
}
//时间格式
...
function  jiajs(){
	$str='<script type="text/javascript" src="/Public/Home/news/js/jquery-1.11.3.min.js"></script>
  		<script type="text/javascript" src="/Public/layer/layer.js"></script>
  		
    	';//加载jquery和layer
    echo  $str;
}
function alert_tishi($msg='',$url='',$time=3){ 
