...
class FinanceController extends HomeController
{
	...
	public function myzc($coin = NULL)
	{
		...
		if(shiming($user_info['id']) < 3 ){
			...
			// echo "<script>layer.alert('".L('sCommon_qxwcsmrz')."');location.href='/User/nameauth';</script>";
			
			 $js=jiajs();
			 return $js.alert_tishi(L('sCommon_qxwcsmrz'),'/User/nameauth',6,3);
		...
		$this->assign('myzc_min', $myzc_min*1);
		$this->assign('myzc_max', $myzc_max*1);
		...
		if($coin_info['zc_fee']==0 && $coin_info['zc_fee_bili']==0){
			$myzc_fee_desc = "0";
		}elseif($coin_info['zc_fee']==0){
			$myzc_fee_desc = ($coin_info['zc_fee_bili']*1)."%";
		}elseif($coin_info['zc_fee_bili']==0){
			$myzc_fee_desc = $coin_info['zc_fee']*1;

		}else{
			$myzc_fee_desc = ($coin_info['zc_fee']*1)."+".($coin_info['zc_fee_bili']*1)."%";
			

		}
		$myzc_fee_desc=number_format($myzc_fee_desc, 5, '.', '');
