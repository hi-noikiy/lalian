...
class IndexController extends HomeController
{
	...
	public function index()
	{
		...
		//底部文章
		//官方公告
		$gfgonggao = M('ArticleType')->where("title='官方公告'")->order('sort asc ,id asc')->find();
		$gglist= M('Article')->where("type='".$gfgonggao['name']."' and status=1")->order('id desc')->limit(7)->select();
		

		//新闻资讯
		$xwzx = M('ArticleType')->where("title='新闻资讯'")->order('sort asc ,id asc')->find();
		$xwlist= M('Article')->where("type='".$xwzx['name']."' and status=1")->order('id desc')->select();

		//交易入门
		$jyrm = M('ArticleType')->where("title='交易入门'")->order('sort asc ,id asc')->find();
		$jylist= M('Article')->where("type='".$jyrm['name']."' and status=1")->order('id desc')->select();

		$this->assign('gfgonggao',$gfgonggao);
		$this->assign('gglist',$gglist);

		$this->assign('xwzx',$xwzx);
		$this->assign('xwlist',$xwlist);

		$this->assign('jyrm',$jyrm);
